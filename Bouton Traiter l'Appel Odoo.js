...
(function() {
    ...
    // =====================================================================
    // Mise en couleur et ajout d'emoji pour Logiciel / Matériel / N2 / RMA
    // =====================================================================
    (function initCategoryHighlighting() {
        try {
            // Styles (couleurs et emojis)
            const style = document.createElement('style');
            style.id = 'ai-category-highlights-style';
            style.textContent = `
            .ai-label-logiciel,
            .ai-label-materiel,
            ...
            /* Pleine largeur pour lignes de tableau (tr) */
            tr.ai-label-logiciel > td, tr.ai-label-logiciel > th { background: rgba(39, 174, 96, 0.12) !important; }
            tr.ai-label-materiel > td, tr.ai-label-materiel > th { background: rgba(155, 89, 182, 0.12) !important; } /* violet */
            tr.ai-label-materiel-n2 > td, tr.ai-label-materiel-n2 > th { background: rgba(231, 76, 60, 0.12) !important; } /* swapped */
            tr.ai-label-rma > td, tr.ai-label-rma > th { background: rgba(230, 126, 34, 0.12) !important; } /* swapped */
            /* Petit span emoji inséré dans le libellé */
            .ai-cat-emoji { display: inline-block; margin-right: 6px; }
            `;
            if (!document.getElementById('ai-category-highlights-style')) {
                document.head.appendChild(style);
            }

            // Détection et application
            const PROCESSED_FLAG = 'ai-cat-highlighted';
            const labelMatchers = [
                { rx: /^(logiciel)(\s*\(|$)/i, className: 'ai-label-logiciel', emoji: '💻' },
                { rx: /^(mat[ée]riel)(\s*\(|$)/i, className: 'ai-label-materiel', emoji: '🔧' },
                { rx: /^(mat[ée]riel\s*n2)(\s*\(|$)/i, className: 'ai-label-materiel-n2', emoji: '🧰' },
                { rx: /^(rma|rma\/sav|rma\/sav tech en cours)/i, className: 'ai-label-rma', emoji: '📦' },
            ];

            function getBestContainer(element) {
                if (!element) return null;
                const container = element.closest('li, .list-group-item, .o_menu_item, .o_accordion_header, .o_accordion_toggle, .o_accordion, .o_kanban_group, .o_kanban_header, .o_control_panel, .o_web_client, .o_list_view, tr, .o_group, .o_inner_group');
            ...
            function applyToElement(el) {
                ...
                if (!text) return;
                for (const { rx, className, emoji } of labelMatchers) {
                    if (rx.test(text)) {
                        const target = getBestContainer(el);
                        const row = target && target.closest ? (target.closest('tr') || target) : target;
                        if (row && !row.classList.contains(PROCESSED_FLAG)) {
                            // nettoyer ancien style si existant
                            row.classList.remove('ai-label-logiciel','ai-label-materiel','ai-label-materiel-n2','ai-label-rma');
                            row.classList.add(className, PROCESSED_FLAG);
                            // Si c'est une ligne de tableau, insérer un emoji devant le libellé
                            if (row.tagName === 'TR' && el && !el.querySelector(':scope > .ai-cat-emoji')) {
                                const span = document.createElement('span');
                        ...
                        return; // une seule correspondance suffit
                    }
                }
            }
            ...
            function scan(root = document) {
                try {
                    const nodes = root.querySelectorAll('li, a, span, div, .o_menu_item, .o_kanban_header, .o_kanban_group, .list-group-item');
            ...
            const triggerScan = () => {
                ...
                debounceTimer = setTimeout(() => scan(document), 250);
            };
            const mo = new MutationObserver((mutations) => {
                ...
                for (const m of mutations) {
                    if (m.addedNodes && m.addedNodes.length) {
                        shouldScan = true;
                        // Application rapide sur les nouveaux nœuds
                        m.addedNodes.forEach(n => {
                            if (n.nodeType === 1) {
                                applyToElement(n);
                                const sub = n.querySelectorAll ? n.querySelectorAll('li, a, span, div') : [];
                                sub.forEach(applyToElement);
                            }
                        });
                    }
                }
